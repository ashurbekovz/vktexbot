...
func GetTesdataConvertationParams() []Testdata {
	return []Testdata{
		...
		},
		{
			...
			Packages: "",
			Params: must.Get(template2img.NewImageParams(
				template2img.MinImageSize(decimal.Zero, decimal.NewFromInt(100)),
			)),
		},
		{
		...
		{
			Name:     "min_image_height_to_font_size_ratio",
			Text:     "text text text text text text text ",
			Packages: "",
			Params: must.Get(template2img.NewImageParams(
				template2img.MinImageHeightToFontSizeRatio(defaultFontSize.Mul(decimal.NewFromInt(4))),
			)),
		},
	}
}
